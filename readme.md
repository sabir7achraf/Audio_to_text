...
Create a database named agentiai:CREATE DATABASE agentiai;
...
Example:curl -X POST -F "file=@sample.wav" -F "id_eleve=1" -F "idTexte=1" http://localhost:5000/upload
...
Example:curl http://localhost:5000/analyze_quality/1
...
Example:curl -X POST http://localhost:5000/retry_transcription/1
...
Example:curl http://localhost:5000/evaluer_lecture_diacritisee/1
...
Example:curl -X POST http://localhost:5000/evaluate_reading/1
...
Example:curl -X POST -H "Content-Type: application/json" -d '{"original_text": "النص الأصلي"}' http://localhost:5000/test_evaluate_reading/1
...
Example:curl -X POST -H "Content-Type: application/json" -d '{"transcription": "نص النسخ", "original_text": "النص الأصلي"}' http://localhost:5000/evaluate_reading_quick
...
Example:curl http://localhost:5000/get_audio_feedback/1
...
Use the UI at http://localhost:5000/ to upload an audio file.
Or via curl:curl -X POST -F "file=@sample.wav" -F "id_eleve=1" -F "idTexte=1" http://localhost:5000/upload


Note the record_id from the response.
...
Add a static file serving route:from flask import send_from_directory
...
Enable debug logging in appo.py to trace issues:print(f"Debug: {variable}")
